...
public class MeleeEnemy : MonoBehaviour
{
    ...
    public float searchDuration = 5f; // Длительность поиска игрока после потери видимости
    ...
    void SearchForPlayer()
    {
        ...
        agent.speed = originalSpeed * 0.7f; // Медленнее во время поиска
        agent.SetDestination(lastKnownPlayerPosition);

        // Проверяем, достигли ли мы позиции
        ...
        // Вращаемся в сторону последней известной позиции
    ...
    void SetRagdoll(bool state)
    {
        ...
        // Включаем/выключаем все Rigidbody в Ragdoll
        foreach (var rb in ragdollRigidbodies)
        {
            rb.isKinematic = !state;
            // Если это не основной Rigidbody, включаем/выключаем
        ...
        // Включаем/выключаем все Collider в Ragdoll
        foreach (var col in ragdollColliders)
        {
            // Если это не основной Collider, включаем/выключаем
        ...
        // Включаем/выключаем основной Collider (обратная логика)
    ...
    void Die()
    {
        ...
        // Включаем Ragdoll вместо анимации смерти
        SetRagdoll(true);

        // Применяем силу к Ragdoll (опционально)
        foreach (var rb in ragdollRigidbodies)
        {
            rb.AddForce(Vector3.up * 2f, ForceMode.Impulse);
        }

        // Отключаем NavMeshAgent и основной Collider
        agent.enabled = false;
