...
public class Shooting : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        transform.localRotation = Quaternion.Lerp(transform.localRotation, originalRotation, Time.deltaTime * recoilRecoverySpeed);
            ...
            // Если игра на паузе (Time.timeScale == 0), не обрабатываем ввод
    ...
    {
        StopAllCoroutines(); // Останавливаем перезарядку, если она идет
        isReloading = false; // Сбрасываем флаг перезарядки
        transform.localPosition = originalPosition; // Возвращаем в исходную позицию
        transform.localRotation = originalRotation; // Возвращаем в исходный поворот

        // Дополнительно: выключаем вспышку, если она активна
